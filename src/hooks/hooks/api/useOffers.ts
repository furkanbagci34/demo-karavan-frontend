...
export const useOffers = () => {
    ...
    const sendOffer = useCallback(
        async (
            ...
            mode?: "detailed" | "summary" | "nameOnly" | "distributor"
        ) => {
            try {
                setLoading(true);
                setError(null);
    ...
    const getOfferProductsByOfferNumber = useCallback(
        async (offerNumber: string): Promise<{ id: number; name: string; image: string; quantity: number }[]> => {
            try {
                setLoading(true);
                setError(null);

                const response = await apiClient.get<{ id: number; name: string; image: string; quantity: number }[]>(
                    API_ENDPOINTS.offers.getProductsByOfferNumber(offerNumber)
                );
                return response;
            } catch (err: unknown) {
                const errorMessage = getErrorMessage(err, "Teklif ürünleri yüklenirken bir hata oluştu");
                setError(errorMessage);
                return [];
            } finally {
                setLoading(false);
